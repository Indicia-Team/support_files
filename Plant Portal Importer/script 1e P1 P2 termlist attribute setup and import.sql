...
values 
('Perennation','Perennation for Plant Portal project',(select id from websites where title='Plant Portal' and deleted=false),now(),1,now(),1,'indicia:perennation');
...
from termlists
where title='Perennation' AND website_id = (select id from websites where title='Plant Portal' and deleted=false);
