...
values 
('BR Habitats','BR Habitats terms for Plant Portal',(select id from websites where title='Plant Portal' and deleted=false),now(),1,now(),1,'indicia:br_habitats');
...
from termlists
where title='BR Habitats' AND website_id = (select id from websites where title='Plant Portal' and deleted=false);
      ...
      LOOP
         ...
         THEN
           ...
           values (
             ...
             (select id from taxa_taxon_list_attributes where caption='BR Habitats' and deleted=false),
