...
```json
PUT occurrence_{{ Indicia warehouse unique name }}_v1?include_type_name=true
{
  "settings": {
    "number_of_shards": 4,
...
```json
PUT occurrence_{{ Indicia warehouse unique name }}_v1
{
  "settings": {
    "number_of_shards": 4,
    "number_of_replicas": 1
...
POST /_aliases
{
  "actions" : [
    { "add" : {
      "index" : "occurrence_{{ Indicia warehouse unique name }}_v1",
    ...
    { "add" : {
      "index" : "occurrence_{{ Indicia warehouse unique name }}_v1",
      "alias" : "occurrence_{{ Indicia warehouse unique name }}_index"
    } }
  ]
}
```
