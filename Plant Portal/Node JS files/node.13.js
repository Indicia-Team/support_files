  ...
  if (!urlParams.get('group_id')) {
      ...
      if (jQuery(this).val() == 15068) {
    	...
    	// Put data entry selection page onto existing rows
      	...
      	jQuery('[name="group+\\:pages\\:\\:1\\:0"] option[value="my-plots-npms-mode:NPMS Mode - my plots"]').prop('selected', true);
      	jQuery('[name="group+\\:pages\\:\\:1\\:1"]').val('My plots'); 
	    // Put admin plot management onto existing row
      	...
      	jQuery('#complex-attr-grid-group-pages').append('<input type="text" name="group+:pages::3:0" value="node/wildflower-data-entry:NPMS Mode - Wildflower">');
	  	jQuery('#complex-attr-grid-group-pages').append('<input type="text" name="group+:pages::3:1" value="Wildflower">');
      	
	  	jQuery('#complex-attr-grid-group-pages').append('<input type="text" name="group+:pages::4:0" value="node/indicator-data-entry:NPMS Mode - Indicator">');
	  	jQuery('#complex-attr-grid-group-pages').append('<input type="text" name="group+:pages::4:1" value="Indicator">');
	  
	  	jQuery('#complex-attr-grid-group-pages').append('<input type="text" name="group+:pages::5:0" value="node/inventory-data-entry:NPMS Mode - Inventory">');
      ...
      if (jQuery(this).val() == 15069) {
      	...
      	// Put user plot list onto existing row
      	jQuery('[name="group+\\:pages\\:\\:1\\:0"] option[value="my-plots-standard-mode:Standard Mode - my plots"]').prop('selected', true);
      	jQuery('[name="group+\\:pages\\:\\:1\\:1"]').val('My plots'); 
