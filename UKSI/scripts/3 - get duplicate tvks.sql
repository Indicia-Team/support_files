...
FROM taxa t
JOIN taxa_taxon_lists ttl ON ttl.taxon_id=t.id AND ttl.deleted=false AND ttl.taxon_list_id=(select uksi_taxon_list_id from uksi.uksi_settings) AND ttl.deleted=false
