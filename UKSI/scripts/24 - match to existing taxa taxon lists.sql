...
-- We'll also grab the meaning ID for which we'll keep AS it is for existing preferred names from
-- UKSI. We also grab the original taxon_meaning_id and original parent_id for all existing names
-- though update them later - grabbing the here just makes it easier to detect changes.
UPDATE uksi.prepared_taxa_taxon_lists pttl
SET id=ttl.id,
  -- Only keep existing meaning for preferred names on UKSI.
  taxon_meaning_id=case when pttl.preferred=true and is_uksi=true then ttl.taxon_meaning_id else null end,
