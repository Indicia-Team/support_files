...
class extension_poms_extensions {
   ...
   * <li><b>editingOnly</b><br/>
   * Do not allow user to enter new records. User is shown a warning instead of the Country drop-down.</li>
   * <li><b>disableFiltering</b><br/>
   * Do not filter the items in the termlists or species list. Can be useful during development</li>  
   *
   * @return string $r
  ...
  public static function limit_termlists_and_species_to_selected_location($auth, $args, $tabAlias, $options) {
    ...
    if (empty($options['editingOnly'])) {
      $options['editingOnly'] = FALSE;
    }
    ...
    if (empty($_GET['location_termlist_and_species_filter']) || empty($_GET['sample_id'])) {
      $r .= self::display_countries_drop_down_or_edits_only_warning($auth['read'], $options['countryTermlistId'], $options['editingOnly']);
    }
    ...
    if (empty($options['disableFiltering']) || $options['disableFiltering'] != TRUE) {
   ...
   * on the page. If editing only mode is enabled, then display a warning instead.
   * 
   * @return array string
   *   HTML for a location select drop-down.
   */
  private static function display_countries_drop_down_or_edits_only_warning($readAuth, $countryTermlistId, $editingOnly) {
    $r = '';
    if (!empty($editingOnly) && $editingOnly == TRUE) {
      $r .= '<em>Please note this page is for editing records only. A sample_id must be supplied.</em>';
    } else {
