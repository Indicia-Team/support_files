...
limit 1)
where id = (select id from taxa_taxon_list_attributes where caption='rarity score' and deleted=false order by id desc limit 1);
...
where ttlav2.taxa_taxon_list_attribute_id = (select id from taxa_taxon_list_attributes where caption='rarity score' and deleted=false order by id desc limit 1) AND ttlav2.deleted=false
...
values (trait_to_import.taxa_taxon_list_id,(select id from taxa_taxon_list_attributes where caption='rarity score' and deleted=false order by id desc limit 1),trait_to_import.insertion_val,1,now(),1,now(),trait_to_import.source);
ELSE 
END IF;
END LOOP;
END
