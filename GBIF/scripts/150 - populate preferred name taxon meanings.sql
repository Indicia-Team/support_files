...
-- All new names will need a taxa_taxon_list_id. 
-- Use the sequence to make this simple.
UPDATE gbif.prepared_taxa_taxon_lists pttl
SET 
  id = nextval('indicia.taxa_taxon_lists_id_seq'::regclass),
  is_new = true
WHERE id IS NULL;

-- Any preferred names that don't already have a taxon_meaning_id will need one.
SELECT 
  nextval('indicia.taxon_meanings_id_seq'::regclass) AS taxon_meaning_id, 
  pttl.id AS taxa_taxon_list_id
